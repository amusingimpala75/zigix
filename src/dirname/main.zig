...
fn process(filename: []const u8) []const u8 {
    return std.fs.path.dirname(filename) orelse if (filename[0] == '/') "/" else ".";
}
...
test "absolute paths" {
    try std.testing.expectEqualSlices(u8, "/foo/bar", process("/foo/bar/baz"));
    try std.testing.expectEqualSlices(u8, "/foo/bar", process("/foo/bar/baz/"));
    try std.testing.expectEqualSlices(u8, "/foo/bar", process("/foo/bar/baz.txt"));
}
...
test "no directory" {
    try std.testing.expectEqualSlices(u8, ".", process("foo_bar_baz.txt"));
}
